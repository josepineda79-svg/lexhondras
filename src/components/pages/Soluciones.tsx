...
import { ChevronDown, Shield, FileText, CheckCircle2, ArrowRight, Lock, MapPin, Search, Edit3, Building2, Landmark, ClipboardList, Scale, Image } from 'lucide-react';
...
export function Soluciones({ onStart, initialTab = 'despachos' }: SolucionesProps) {
  ...
  const solutions = {
    despachos: {
      ...
      subtitle: 'Multiplica la capacidad de tu despacho sin renunciar al rigor jurídico.',
      imageFile: 'despachos_de_abogados.jpg',
      imageUrl: 'https://images.unsplash.com/photo-1556761175-4b46a572b786?auto=format&fit=crop&q=80&w=800',
      icon: Scale,
      quote: '"Soy LexHonduras, tu copiloto de IA experto en derecho hondureño. Mientras yo analizo miles de folios y preparo tus borradores en segundos, tú te centras en ganar el caso y cuidar a tus clientes. Sin complicaciones y tan fácil como usar WhatsApp."',
      badge: 'Solución para Despachos'
    },
    departamentos: {
      id: 'departamentos',
      title: 'Departamentos jurídicos y empresas',
      subtitle: 'Agiliza las respuestas legales internas e impulsa la eficiencia corporativa.',
      imageFile: 'departamentos_juridicos.jpg',
      imageUrl: 'https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=80&w=800',
      icon: Building2,
      quote: '"Responde las consultas legales de tu empresa en minutos. LexHonduras analiza contratos, evalúa riesgos de cumplimiento y genera dictámenes basados en la legislación hondureña."',
      badge: 'Solución Corporativa'
    },
    asesorias: {
      id: 'asesorias',
      title: 'Asesorías y Gestorías',
      subtitle: 'Atiende a más clientes con máxima velocidad en trámites fiscales, laborales y contables.',
      imageFile: 'asesorias_y_gestorias.jpg',
      imageUrl: 'https://images.unsplash.com/photo-1554200876-56c2f25224fa?auto=format&fit=crop&q=80&w=800',
      icon: ClipboardList,
      quote: '"Calcula prestaciones laborales, redacta contratos y prepara documentación oficial para tus clientes con las fórmulas exactas de la Secretaría de Trabajo y el Banco Central de Honduras."',
      badge: 'Solución para Gestorías'
    },
    administraciones: {
      id: 'administraciones',
      title: 'Administraciones públicas',
      subtitle: 'Garantiza la seguridad procesal y la transparencia en dictámenes y expedientes públicos.',
      imageFile: 'administraciones_publicas.jpg',
      imageUrl: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=80&w=800',
      icon: Landmark,
      quote: '"El pilar de las instituciones públicas. Automatiza la revisión de reglamentos, acuerdos gubernamentales y La Gaceta con un sistema seguro y libre de sesgos."',
      badge: 'Solución Institucional'
    }
  };

  const current = solutions[activeTab as keyof typeof solutions] || solutions.despachos;
  ...
  return (
    <div className="pt-32 pb-20 w-full overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 mb-12">
        <div className="flex items-center justify-center gap-3 overflow-x-auto pb-4 hide-scrollbar">
          {Object.values(solutions).map((sol) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2.5 px-6 py-3 rounded-full font-medium text-sm transition-all shrink-0 ${
                  isActive
                    ? 'bg-[#1a7484] text-white shadow-md shadow-[#1a7484]/20'
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 mb-24">
        <div className="flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#e8f1f2] border border-[#1a7484]/20 rounded-full text-xs font-semibold text-[#1a7484] mb-4">
              <span>{current.badge}</span>
            </div>
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-serif text-slate-900 leading-tight mb-6">
              {current.title}: <span className="text-[#1a7484] italic">{current.subtitle}</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              <button 
                onClick={onStart}
                className="w-full sm:w-auto bg-[#1a7484] text-white px-8 py-3.5 rounded-full font-medium hover:bg-[#145d6a] transition-colors shadow-md"
              >
                Solicitar una demo
              </button>
              
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-mono text-slate-600">
                <Image className="w-4 h-4 text-[#1a7484]" />
                <span>Imagen: {current.imageFile}</span>
              </div>
            </div>
          </div>
          <div className="flex-1 relative">
            ...
            <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-slate-200">
              ...
              />
              <div className="absolute bottom-4 left-4 bg-slate-900/80 backdrop-blur-md text-white text-xs px-3 py-1.5 rounded-lg font-mono flex items-center gap-2">
                <Image className="w-3.5 h-3.5 text-[#6ad2de]" />
                <span>{current.imageFile}</span>
              </div>
            </div>
          </div>
        </div>
